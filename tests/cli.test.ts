...
function runWithExit(args: string): { stdout: string; stderr: string; code: number } {
    const cmd = `node ${CLI} ${args}`;
    try {
...
beforeAll(() => {
    // Ensure CLI is built
    execSync('npx tsup src/cli.ts --format esm --out-dir dist', {
        cwd: resolve(__dirname, '..'),
        stdio: 'ignore',
        timeout: 30_000,
    });
...
describe('CLI — get', () => {
    ...
    it('returns default for missing path', () => {
        expect(run(`get ${configJson} "missing.path" --default fallback`)).toBe('fallback');
    });
...
describe('CLI — set', () => {
    it('sets a value and outputs JSON', () => {
        const out = run(`set ${configJson} "database.port" 3306 --to json --pretty`);
        const parsed = JSON.parse(out);
        expect(parsed.database.port).toBe(3306);
    });

    it('sets a string value', () => {
        const out = run(`set ${configJson} "database.host" '"newhost"' --to json`);
        const parsed = JSON.parse(out);
...
describe('CLI — remove', () => {
    it('removes a path and outputs JSON', () => {
        const out = run(`remove ${configJson} "database.port" --to json --pretty`);
        const parsed = JSON.parse(out);
...
describe('CLI — has', () => {
    it('exits 0 when path exists', () => {
        const { stdout, code } = runWithExit(`has ${configJson} "database.host"`);
        expect(stdout).toBe('true');
        expect(code).toBe(0);
    });

    it('exits 1 when path missing', () => {
        const { stdout, code } = runWithExit(`has ${configJson} "missing.path"`);
...
describe('CLI — validate', () => {
    ...
    it('validates valid data with json output', () => {
        const out = run(`validate ${configJson} --schema ${configSchema} --format json`);
    ...
    it('reports validation errors with text output', () => {
        const { stderr, code } = runWithExit(`validate ${configJson} --schema ${configFailSchema}`);
        expect(code).toBe(1);
        expect(stderr).toContain('missing_field');
    });

    it('reports validation errors with json output', () => {
        const { stdout, code } = runWithExit(`validate ${configJson} --schema ${configFailSchema} --format json`);
        expect(code).toBe(1);
...
describe('CLI — error handling', () => {
    ...
    it('reports missing file', () => {
        const { stderr, code } = runWithExit('get /nonexistent/file.json "key"');
        expect(code).toBe(1);
